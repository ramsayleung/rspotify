...
impl ApiError {
    fn from(status_code: reqwest::StatusCode, headers: reqwest::header::HeaderMap) -> Self {
        match status_code {
            StatusCode::UNAUTHORIZED => ApiError::Unauthorized,
            StatusCode::TOO_MANY_REQUESTS => {
                if let Ok(duration) = headers[reqwest::header::RETRY_AFTER].to_str() {
...
impl Spotify {
    ...
    async fn internal_call(
        ...
        // TODO it only needs to clone when failed, but now clones everytime.
        let headers = response.headers().clone();
        match response.error_for_status() {
            Ok(res) => Ok(res.text().await?),
            Err(_) => Err(failure::Error::from(ApiError::from(status, headers))),
        }
    }
    ///send get request
    async fn get(
    ...
    // fn search<L: Into<Option<u32>>, O: Into<Option<u32>>>(
    //     &self,
    //     q: &str,
    //     _type: SearchType,
    //     limit: L,
